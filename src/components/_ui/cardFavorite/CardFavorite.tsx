...
const removeFavoriteItem = (item:any) => {
  try {
    ...
    dispatch(catsActions.removeItemFavorite(item.id));
  } catch (error) {
  ...
  return (
    <div className='card-f' style={{"backgroundImage": `url("${item.image.url}")`}} onClick={() => removeFavoriteItem(item)}>
      <div className='card-f__decoration'>
        { item.isFavorite ?
